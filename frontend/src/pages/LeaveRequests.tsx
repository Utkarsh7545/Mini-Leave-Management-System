  ...
  const fetchLeaveRequests = async () => {
    try {
      ...
      const response = await api.get(
        `/leaves/requests?status=${filter === "all" ? "" : filter}&limit=50`
      );
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span
                      className={`${getStatusBadgeClass(leave.status)} ml-2`}
                    >
                      {leave.status}
                    </span>
                  </div>
            ...
            <div className="p-6">
              ...
              <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                ...
                <p className="text-sm text-gray-600">
                  ...
                  {formatDate(selectedLeave.startDate)} -{" "}
                  {formatDate(selectedLeave.endDate)} ({selectedLeave.totalDays}{" "}
                  day{selectedLeave.totalDays !== 1 ? "s" : ""})
                </p>
              </div>

              <form
                onSubmit={handleSubmit(handleReviewLeave)}
                className="space-y-4"
              >
                <div>
